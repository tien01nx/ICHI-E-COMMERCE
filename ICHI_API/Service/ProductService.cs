namespace ICHI_API.Service
{
  ...
  public class ProductService : IProductService
  {
    ...
    public IQueryable<Product> FilterProducts(IQueryable<Product> query, string categoryName, List<string> color, List<string> trademark, decimal? priceMin, decimal? priceMax)
    {
    ...
    public Helpers.PagedResult<ProductDTO> GetProductInCategory(string categoryName, int pageSize, int pageNumber, string sortDir, string sortBy, List<string>? colors, List<string>? trademarkName, decimal? priceMin, decimal? priceMax, out string strMessage)
    {
      strMessage = string.Empty;
      try
      {
        var query = _unitOfWork.Product.GetAll().AsQueryable();
        query = FilterProducts(query, categoryName, colors, trademarkName, priceMin, priceMax);
        var orderBy = $"{sortBy} {(sortDir.ToLower() == "asc" ? "ascending" : "descending")}";
        query = query.OrderBy(orderBy);
