namespace ICHI_API.Service
{
  ...
  public class ProductService : IProductService
  {
    ...
    public ProductService(IUnitOfWork unitOfWork, PcsApiContext pcsApiContext)
    {
      _unitOfWork = unitOfWork;
      _db = pcsApiContext;
    }
    ...
    public Helpers.PagedResult<ProductDTO> GetAll(string name, int pageSize, int pageNumber, string sortDir, string sortBy, out string strMessage)
    {
      ...
      try
      {
        ...
        var query = _unitOfWork.Product.GetAll(u => u.isDeleted == false).AsQueryable();
    ...
    public Product Create(Product product, List<IFormFile>? files, out string strMessage)
    {
      ...
      try
      {
        ...
        var model = _unitOfWork.Product.Get(x => x.Id == product.Id, tracked: true);

        // nếu tên sản phẩm khác với tên truyền vào thi kiem tra xem tên sản phẩm đã tồn tại chưa, nếu đã tồn tại thì không cho thêm
        if (model.ProductName != product.ProductName)
        {
          var checkProduct = _unitOfWork.Product.Get(x => x.ProductName == product.ProductName);
          if (checkProduct != null)
          {
    ...
    public bool Delete(int id, out string strMessage)
    {
      ...
      try
      {
        var data = _unitOfWork.Product.Get(u => u.Id == id && !u.isDeleted, tracked: true);
        if (data == null)
        {
          strMessage = "Sản phẩm không tồn tại";
          return false;
