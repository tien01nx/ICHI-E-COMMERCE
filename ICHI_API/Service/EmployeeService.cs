    ...
    public class EmployeeService : IEmployeeService
    {
        ...
        public Helpers.PagedResult<Employee> GetAll(string name, int pageSize, int pageNumber, string sortDir, string sortBy, out string strMessage)
        {
            ...
            try
            {
                ...
                //var query = _unitOfWork.Employee.GetAll().AsQueryable().Where(u => u.isDeleted == false).OrderByDescending(u => u.ModifiedDate);
        ...
        public Employee Create(Employee model, out string strMessage)
        {
            ...
            try
            {
                ...
                if (_unitOfWork.Employee.ExistsBy(u => u.UserId == model.UserId))
                    throw new BadRequestException(EMAILEXIST);
                if (_unitOfWork.Employee.ExistsBy(u => u.PhoneNumber == model.PhoneNumber))
