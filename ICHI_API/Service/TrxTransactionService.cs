    ...
    public class TrxTransactionService : ITrxTransactionService
    {
        ...
        public ShoppingCartVM Update(UpdateTrxTransaction model, out string strMessage)
        {
            ...
            try
            {
                ...
                if (data.PaymentStatus == AppSettings.PaymentStatusPending && data.PaymentTypes == AppSettings.PaymentStatusPending || data.PaymentStatus == AppSettings.PaymentStatusPending)
                {
        ...
        public List<MoneyMonth> getMonneyTotalByMonth(int year, out string strMessage)
        {
            ...
            try
            {
                ...
                for (int i = 1; i <= 12; i++)
                {
                    ...
                    var inventoryReceiptDetail = _unitOfWork.InventoryReceiptDetail.GetAll(includeProperties: "InventoryReceipt");
                    var inventoryReceipt = inventoryReceiptDetail.Where(u => u.InventoryReceipt.CreateDate.Month == i && u.CreateDate.Year == year);
                    moneyMonth.TotalRealAmount = inventoryReceipt.Sum(u => u.Quantity * u.Price);
                    moneyMonth.Month = i;
            ...
            try
            {
                ...
                for (int i = 1; i <= 12; i++)
                {
                    ...
                    foreach (var item in order)
                    {
                        ...
                        var amountInventory = _unitOfWork.InventoryReceiptDetail.Get(u => u.ProductId == item.ProductId && u.BatchNumber == item.BatchNumber);

                        if (amountInventory != null)
                        {
                            revenue += (item.Price * item.Quantity) - (amountInventory.Price * item.Quantity);
                        }
                        else
                        {
        ...
        public bool CancelOrder(UpdateOrderStatus model, out string strMessage)
        {
            strMessage = string.Empty;
            try
            {
                var data = _unitOfWork.TrxTransaction.Get(u => u.Id == model.Id);
                if (data == null)
                {
                    throw new BadRequestException(TRXTRANSACTIONNOTFOUNDORDER);
                }
                if (data.OrderStatus == AppSettings.PaymentStatusPending || data.OrderStatus == AppSettings.StatusOrderOnHold)
                {
                    data.OrderStatus = AppSettings.StatusOrderCancelled;
                    data.CancelledDate = DateTime.Now;
                    data.Notes = model.CancelReason;
                    _unitOfWork.TrxTransaction.Update(data);
                    strMessage = "Hủy đơn hàng thành công";
                    _unitOfWork.Save();
                }
                else
                {
